use crate::{assert_float_eq, math::vector::Vec3, vec3};
...
pub fn vec3_should_return_vector_with_all_arguments() {
    ...
    // Assert
    assert_float_eq!(Vec3 vec, Vec3::new(1.0, 2.0, 3.0));
}
...
pub fn vec3_should_fill_vector_with_argument() {
    ...
    let vec = vec3!(1.0);
    // Assert
    assert_float_eq!(Vec3 vec, Vec3::new(1.0, 1.0, 1.0));
}
...
pub fn cross_should_return_cross_product(lhs: Vec3, rhs: Vec3, expected: Vec3) {
    ...
    // Assert
    assert_float_eq!(Vec3 expected, result);
}
