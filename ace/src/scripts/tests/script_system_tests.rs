...
pub fn run_should_run_scripts_on_entity() {
    ...
    let scripts = Component::Scripts(vec![
        spy_script.clone(),
        spy_script.clone(),
        spy_script.clone(),
    ]);
    entities.add_entity(vec![scripts]);
    // Act
...
pub fn run_should_run_script_on_all_entities() {
    ...
    let spy_script = Box::new(SpyScript::new());
    let mut entities = Entities::empty();
    entities.add_entity(vec![Component::Scripts(vec![spy_script.clone()])]);
    entities.add_entity(vec![Component::Scripts(vec![spy_script.clone()])]);
    entities.add_entity(vec![Component::Position(Default::default())]); // Filler
    entities.add_entity(vec![Component::Scripts(vec![spy_script.clone()])]);
    // Act
    sut.run(&mut entities, &[]);
    // Assert
...
pub fn run_should_update_entity_with_returned_entity() {
    ...
    let mut entities = Entities::empty();
    entities.add_entity(vec![Component::Scripts(vec![add_position_script.clone()])]);
    // Act
    sut.run(&mut entities, &[]);
    // Assert
...
pub fn run_should_update_existing_component_with_returned_entity() {
    ...
    entities.add_entity(vec![
        Component::Scripts(vec![add_position_script.clone()]),
...
impl Script for AddPositionScript {
    fn run(&self, _: &[&Component], _: &[Input]) -> Vec<Component> {
